...
{
	struct Ext2FSInode* inodo = (struct Ext2FSInode*) malloc(_superblock->inode_size);
	int block_size = 1024 << _superblock->log_block_size;
	
	unsigned int numero_de_grupo = blockgroup_for_inode(inode_number);
	struct Ext2FSBlockGroupDescriptor* descriptor_de_grupo = block_group(numero_de_grupo);
	//get_block_address(numero_de_grupo);
	unsigned int indice_inodo = blockgroup_inode_index(inode_number); 
	
	unsigned int inode_table = descriptor_de_grupo->inode_table;

	unsigned bloque_tabla_inodo = (indice_inodo * _superblock->inode_size) / block_size;

	int inodes_per_block = block_size/(_superblock -> inode_size);
	unsigned int indice_bloque_tabla_inodo = indice_inodo % inodes_per_block;
	
	unsigned char* buffer = (unsigned char*) malloc(block_size);
	read_block(inode_table + bloque_tabla_inodo, buffer);

	memcpy(inodo, buffer + indice_bloque_tabla_inodo * _superblock->inode_size, _superblock->inode_size);
	//inodo = copy(inodo);
	//inodo = (Ext2FSInode*) inodo;
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number){
	...
	int block_size = 1024 << _superblock->log_block_size;
	int cantidad_de_entradas_en_un_bloque = block_size / 4; // = 256
	unsigned char* tabla_indireccion_simple_uno = (unsigned char*) malloc(1024);
	...
	// INDIRECCION SIMPLE 
	else if (block_number < cantidad_de_entradas_en_un_bloque + 12){ // +-1
		read_block(inode->block[12],tabla_indireccion_simple_uno);
		puntero = tabla_indireccion_simple_uno[block_number - 12];
		 
	} else if (block_number < 65803){ // INDIRECCION DOBLE
		int tabla_indireccion_simple = (block_number - 268) / cantidad_de_entradas_en_un_bloque;
		read_block(inode->block[13], tabla_indireccion_simple_uno);
		read_block(tabla_indireccion_simple_uno[tabla_indireccion_simple],tabla_indireccion_simple_uno);
		puntero = tabla_indireccion_simple_uno[(block_number - 268) % 256]; // 268= indices anteriores, 12 directos + 256 de la primera tabla 
		
	}
	free(tabla_indireccion_simple_uno);
	return puntero;
}
